 ...
 */
package org.neodatis.odb.core.layers.layer3.bio;
 ...
 * 
 */
public abstract class MultiBufferedIO implements BufferedIO {
	...
	/** A boolean value to check if read&writes are using buffer */
	...
	public MultiBufferedIO(IO io, int nbBuffers, String name, int bufferSize, boolean canWrite, boolean debug) {
		this.nbBuffers = nbBuffers;
		this.bufferSize = bufferSize;
		multiBuffer = new MultiBufferVO(nbBuffers, bufferSize);
		this. io = io;
		currentPositionWhenUsingBuffer = 0;
		currentPositionForDirectWrite = 0;
	...
	public int manageBufferForNewPosition(long newPosition, int readOrWrite, int size) {
		...
		if (readOrWrite == READ && newPosition >= length) {
			...
			throw new NeoDatisRuntimeException(NeoDatisError.END_OF_FILE_REACHED.addParameter(newPosition).addParameter(length));
		}
	...
	public byte[] readBytes(byte[] bytes, int position, int size) {
		currentPositionWhenUsingBuffer = position;
	...
	public void writeBytes(byte[] bytes) {
		...
		if (bytes.length > bufferSize) {
			// throw new NeoDatisRuntimeException(session,"The buffer has a size of "
	...
	public void flushBuffer(int bufferIndex) {
		...
		if (buffer != null && multiBuffer.hasBeenUsedForWrite(bufferIndex)) {
			...
			try{
				io.write(buffer, 0, bufferSizeToFlush);	
			}catch(IOException e){
