...
/**
 * @author olivier
 * TODO adds a cache for already read blocks
 */
public class BlockReader {
	...
	public BlockReader(NdfsFile file) {
		this.file = file;
		this.blockSize = file.getNdfs().getConfig().getBlockSize();
	...
	public Block readBlock(long blockIdToRead) {
		try{
			...
			int size = raf.read(block.bytes);
			/* don t throw exception , it can happen for new files
			if(size!=blockSize){
				throw new NdfsException(String.format("Reading block id %d and its size is %d instead of %d", blockIdToRead, size, blockSize));
			}*/
			return block;
