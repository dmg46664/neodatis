...
public class EqualCriterion extends AbstractCriterion {
	...
	public boolean match(Object valueToMatch) {
		...
		if (isCaseSensitive) {
			...
			try{
				...
				throw new NeoDatisRuntimeException(NeoDatisError.EQUAL_CRITERIA_ON_OBJECT_WITH_WRONG_ATTRIBUTE_TYPE
		...
		if (!canUseCaseInsensitive) {
			throw new NeoDatisRuntimeException(NeoDatisError.QUERY_ATTRIBUTE_TYPE_NOT_SUPPORTED_IN_IEQUAL_EXPRESSION.addParameter(valueToMatch.getClass().getName()));
		}
	...
	public void ready() {
		if (!objectIsNative) {
			...
			SessionEngine engine = getQuery().getSessionEngine();
			if (engine == null) {
			...
			oid = engine.getObjectOid(criterionValue, false);
