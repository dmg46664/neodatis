...
import org.neodatis.odb.main.SameVMODBClient;
...
public class ODBServerImpl implements OdbRunnable, ODBServer {
	...
	public void startServer() throws IOException {
		...
		if (config.logServerStartupAndShutdown()) {
			DLogger.info("NeoDatis ODB Server [version=" + Release.RELEASE_NUMBER + " - build=" + Release.RELEASE_BUILD + " - "
					+ Release.RELEASE_DATE + "] running on port " + port);
			if (bases.size() != 0) {
